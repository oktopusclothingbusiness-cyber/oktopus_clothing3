...
export async function formatCategoryForMobile(cat, db) {
  ...
  if (matchingProducts.length > 0) {
    featuredProducts = matchingProducts.slice(0, 3).map((p) => {
  ...
  if (Array.isArray(cat.featured_products) && cat.featured_products.length > 0) {
    featuredProducts = cat.featured_products.map((fp, idx) => ({
